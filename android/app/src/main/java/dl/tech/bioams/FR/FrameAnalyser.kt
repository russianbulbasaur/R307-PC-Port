...
import dl.tech.bioams.FR.model.MaskDetectionModel
...
private var callback:android.os.Handler) : ImageAnalysis.Analyzer {
            ...
            for (face in faces) {
                try {
                    ...
                    if (maskLabel == maskDetectionModel.NO_MASK) {
                        ...
                        Log.e( "","Average score for each user : $nameScoreHashmap" )
                        ...
                        Log.e( "","Person as $bestScoreUserName" )
                        var data = Bundle();
                        data.putString("name",bestScoreUserName);
                        var message = Message()
                        message.data = data;
                        callback.dispatchMessage(message)
                        predictions.add(
                            Prediction(
                                face.boundingBox,
                                bestScoreUserName ,
